    ...
    <div v-if="expanded" :class="isLeaf ? 'border-t border-secondary-200 p-4' : 'border-t border-secondary-200 p-2 space-y-2'">
      ...
      <div v-if="!isLeaf" class="ml-4 space-y-2">
        <OrgGroup
          ...
          :node="child"
          :members="members"
          @open-role-change="(u: User) => emit('open-role-change', u)"
        />
      </div>

      <!-- 成员卡片网格：叶子节点直接显示,非叶子节点如有本节点成员也显示 -->
      <div v-if="leafMembers.length > 0">
        <div v-if="!isLeaf" class="mb-2 mt-1 px-1 text-xs font-medium text-secondary-500">
          本部门成员 ({{ leafMembers.length }})
        </div>
      ...
      <div v-else-if="isLeaf" class="py-4 text-center text-xs text-secondary-400">
        暂无成员
      </div>
    </div>
  </div>
...
const emit = defineEmits<{
  (e: 'open-role-change', user: User): void;
}>();
