    ...
    <div class="flex h-full flex-col bg-secondary-800 text-white">
      ...
      <div class="flex h-16 items-center justify-center border-b border-secondary-700">
        <div class="flex items-center gap-3">
          ...
          <transition name="fade">
            <span v-if="!uiStore.sidebarCollapsed" class="text-xl font-bold">{{ $t('app.shortName') }}</span>
          </transition>
        </div>
      </div>
      ...
      <div class="border-t border-secondary-700 p-3">
        ...
        >
          ...
          <transition name="fade">
            <span v-if="!uiStore.sidebarCollapsed" class="font-medium">{{ $t('sidebar.toggle') }}</span>
          </transition>
        </button>
      </div>
    </div>
...
const menuItems = computed(() => [
  ...
  {
    name: 'myTasks',
    label: t('routes.myTasks'),
    to: '/my-tasks',
    icon: '<svg class="h-6 w-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>'
  },
  {
    name: 'projects',
    label: t('routes.projectList'),
    to: '/projects',
  ...
  {
    name: 'weekly-reports',
    label: t('routes.weeklyReports'),
    to: '/weekly-reports',
    icon: '<svg class="h-6 w-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>'
  },
  {
    name: 'team',
    label: t('routes.team'),
  ...
  ...(permissionStore.currentRole === 'admin' ? [{
    name: 'admin-access-heatmap',
    label: t('routes.adminAccessHeatmap'),
    to: '/admin/access-heatmap',
