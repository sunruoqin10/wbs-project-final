        ...
        <template v-else>
          ...
          <div>
            <label class="mb-2 block text-sm font-medium text-secondary-700">
              {{ t('handover.projectScope') }}
            </label>
            <div
              v-if="(preview?.candidateProjects || []).length === 0"
            ...
            >
              <label
                v-for="p in preview?.candidateProjects"
                :key="p.id"
            ...
            <p v-if="selectedProjectNames.length > 0" class="mt-1 text-xs text-secondary-500">
              {{ t('handover.selectedCount', { count: selectedProjectNames.length }) }}
            </p>
          </div>
...
import type { HandoverPreviewResponse, PmHandoverResponse } from '@/types/handover';

const { t } = useI18n();

/**
 * PM 交接对话框(2026-06-16 PM/Dept-PM 变更方案)
 *
 * Props:
 *   - visible           : 控制显示
 *   - userId            : 离任 PM 用户 ID
 ...
 *   - success           : 提交成功,带 PmHandoverResponse
 */
...
const emit = defineEmits<{
  ...
  (e: 'success', r: PmHandoverResponse): void;
}>();
...
const selectedProjectNames = computed(() => {
  const projects = preview.value?.candidateProjects || [];
...
watch(
  ...
  async v => {
    ...
    loading.value = true;
    try {
      const r = await handoverService.preview(props.userId, 'PM_HANDOVER');
      preview.value = r;
      // 默认勾选所有 candidateProjects
      selectedProjectIds.value = (r?.candidateProjects || []).map(p => p.id);
    } catch (e: any) {
...
const handleConfirm = async () => {
  ...
  if (selectedProjectIds.value.length === 0) {
    errorMsg.value = t('handover.validation.needProject');
    return;
  }
  errorMsg.value = '';
  submitting.value = true;
  try {
    const r = await handoverService.handoverPm(props.userId, {
