...
| **17. 加班记录编辑权限**    | `OvertimeService.updateRecord()` 只允许待审批状态修改，但已拒绝的申请应该允许重新提交 | ⚠️ 用户体验不好 | ✅ 已解决 |
...
1. **修改 updateRecord 方法** - 允许 `pending`（待审批）和 `rejected`（已拒绝）状态的记录修改

2. **重新提交逻辑** - 当已拒绝状态的记录被修改后，自动重置为待审批状态：
   - 状态重置为 `pending`
   - 清空审批人ID
   - 清空审批时间
   - 清空拒绝原因

3. **修改 deleteRecord 方法** - 允许 `pending` 和 `rejected` 状态的记录删除

**修改的文件：**

* `backend/src/main/java/com/wbs/project/service/OvertimeService.java`
